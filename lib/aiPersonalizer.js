...
//
// Why Groq?
//   - Free tier: 30 req/min, 14,400 req/day â†’ plenty for 12 msg/day workflow
...
//   3. Inject lead's actual data â€” niche, name, notes â€” for relevance
...
function matchNicheCategory(niche, name = '', notes = '') {
  ...
  for (const key of orderedKeys) {
    const variants = key === 'real_estate' ? ['real_estate', 'real estate', 'realty', 'realtor', 'property'] : [key]
...
//
// FUTURE: When user adds non-India leads, this same logic extends â€”
...
function isIndianLead(lead) {
  ...
  return false  // non-Indian (US/UK/UAE/etc.) â†’ use English
}
...
function buildPrompt(lead) {
  ...
  // AI is told to pick ONE specific pain and ONE related service â€” not list all.
...
This is critical â€” the message MUST be written in natural Hinglish like a real Indian person typing on WhatsApp.

Hinglish examples (study these patterns):
- "Main Sami hoon â€” chhote businesses ke liye automation systems banata hoon"
...
2. Introduces ${SENDER_NAME} as someone who builds automation for businesses (NOT only WhatsApp â€” could be any automation relevant to their industry)
3. Mentions ONE specific pain point relevant to their business AND ONE specific automation service that solves it
4. Asks if they'd want a 2-minute walkthrough â€” they should reply "yes" or "show me" to engage
5. Ends with "â€” ${SENDER_NAME}" signature on its own line

STRICT RULES:
- NO emojis whatsoever
- NO "Reply H or B" or language-switch prompts
- NO pricing mentioned
- NO fake statistics or made-up client names â€” keep it honest and grounded
- NO marketing buzzwords like "leverage", "synergy", "game-changer", "revolutionize"
- NO ALL-CAPS, NO excessive exclamation marks
- DO NOT default to mentioning "WhatsApp automation" for every lead â€” vary services based on what fits the niche best
...
function cleanAIOutput(text) {
  ...
  // Strip emojis (covers most ranges â€” astral plane chars)
  ...
  if (!cleaned.toLowerCase().includes(SENDER_NAME.toLowerCase())) {
    // AI forgot to sign â€” add it
    cleaned += `\n\nâ€” ${SENDER_NAME}`
  }

  // Validate length â€” if too long (over 200 words), reject (use fallback)
...
// Language-aware fallback â€” matches the same Hinglish/English split as AI prompts.
// This is the safety net â€” same quality as Layer 1 fixed templates.
...
Main ${SENDER_NAME} hoon â€” chhote businesses ke liye automation systems banata hoon (WhatsApp, email, CRM, lead generation, AI agents, workflow automation â€” jo bhi business ko chahiye).

${nicheText} businesses ke saath aksar ek common problem hoti hai â€” ${firstPain}. Iske liye ${firstService} ek simple solution hai, jo manually time-consuming kaam ko automate kar deta hai.
...
I'm ${SENDER_NAME} â€” I build business automation systems for small companies. This includes WhatsApp automation, email automation, lead generation, CRM workflows, AI agents, and custom integrations.
...
export async function generatePersonalizedMessage(lead) {
  ...
  // No API key â†’ use fallback (Layer 1 templates)
  if (!GROQ_KEY) {
  ...
  try {
    ...
    // Race against timeout â€” don't let one lead block the whole batch
    ...
    const resp = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        // llama-3.3-70b-versatile â€” better quality than 8b-instant for nuanced writing
...
export async function generateMessagesForLeads(leads, opts = {}) {
  ...
  const delayMs   = opts.delayMs   || 2000   // 2s between batches â†’ safe under 30/min
  const results = []
